...
from keras.layers import Activation, Dropout, Flatten, concatenate, Maximum, Add, Multiply
...
model_path = 'D:/636/multiply_model.h5'
graph_path = 'D:/636/model_m_v1.png'
...
def Model_Design():
    ...
    fc_1 = Dense(units=1028, activation='relu', kernel_regularizer=l2(0.001))(feature)
...
for i in range(range_num):
    ...
    m = list(a)
    predict_label = model.predict(m)
    json_label[i][0]= predict_label[0][31]
    print(predict_label[0][31])
#print(json_label)
#End Here

#Time Function:
output_json_file = np.zeros((range_num,1))
t = 3
per_cus = 3 / data1_frames
for i in range(range_num):
    output_json_file[i][0] = per_cus * 30 + (i + 1) * per_cus
...
#output_json_file=['Brush Teeth:',output_json_file.tolist()]
#json_path = 'D:/636/S001C002P006R002A003_maximum.json'
#with open(json_path,'w') as f:
        #json.dump(output_json_file, f)
...
#tst_path = 'D:/ChromeDownload/cross_subject/Train.hdf5'
#data_name = 'S001C001P001R001A001'
#with h5py.File(tst_path,'r') as read_file:
            #single_train_data = read_file[data_name]
            #print(single_train_data)
            #output_cursors = []
            #all_out = single_train_data[0, :, :, :] - single_train_data[1,:,:,:]
            #print(all_out)
...
file = open('D:/ChromeDownload/cross_subject/action_name.txt','w')
file.write(str(action_name))
file.close()
