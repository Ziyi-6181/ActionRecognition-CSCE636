...
from keras.layers import Activation, Dropout, Flatten, concatenate, Maximum, Add, Multiply, Average
...
#train_path = 'D:/Train.hdf5'
#tst_path = 'D:/Test.hdf5'
#weight_path = 'D:/636/Part8/multiply_weights.h5'
#model_path = 'D:/636/Part8/multiply_model.h5'
#graph_path = 'D:/636/Part8/model_m_v1.png'
...
def Model_Design():
    ...
    up_feature = Multiply()([up_feature_0, up_feature_1]) # Change Here: Add, Maximum, Average, Multiply
    down_feature = Multiply()([down_feature_0, down_feature_1])

    feature = concatenate([up_feature, down_feature])

    fc_1 = Dense(units=1024, activation='relu', kernel_regularizer=l2(0.001))(feature)
    fc_1 = Dropout(0.5)(fc_1)
    
    fc_2 = Dense(units=256, activation='relu', use_bias=True)(fc_1)

    fc_3 = Dense(units=128, activation='relu', use_bias=True)(fc_2)
...
# Main Function To Train My Model:
...
#Anction Namelist:
action_name = ['Drink Water', 'Eat Meal', 'Brush Teeth','Brush Hair','Drop','Pick Up','Throw',
              ...
              'Take off Jacket', 'Put on a Shoe', 'Take off a Shoe', 'Put on Glasses', 'Take off Glasses', 
               'Put on a Hat', 'Take off a Hat', 'Cheer Up', 'Hand Waving', 'Kicking Something', 'Reach into Pocket',
              'Hopping', 'Jump Up', 'Phone Call', 'Play with Phone', 'Type on Keyboard', 'Point to Something',
               'Taking a Selfie', 'Check Time', 'Rub Two Hands', 'Nod Head', 'Shake Head', 'Wipe Face', 'Salute',
              'Put Palms together', 'Cross Hands', 'Sneeze', 'Staggering', 'Falling down', 'Headache', 'Chest Pain',
              'Back Pain', 'Neck Pain', 'Nausea Vomiting', 'Fan Self', 'Punch', 'Kicking', 'Pushing', 'Pat on Back',
              'Point Finger', 'Hugging', 'Giving Object', 'Touch Pocket', 'Shaking Hands', 'Walking Towards', 'Walking Apart']

#----Changing Index Here to see whether the action is brush teeth-----

test_data_number = int(input("Input The Video Index:"))# Test Here, brush teeth: 2, 61, 180, not brush teeth: 4, 200， you can input every number, there are 16324 test videos.
#print("The Video Name is:", namelist[test_data_number])

#--------------------------------------------------------------------
test_data_number = test_data_number + 59 - 1
...
k = 0
for i in range(60):
    if predicted[0][i] == yhat:
        k = i

print("The .skeleton File Name:",namelist[test_data_number], "\nThe Action is:", action_name[k],
      "\nThe Probability of", action_name[k], "is:", predicted[0][k])
...
json_label = np.zeros((range_num,2))
action_index = test_data_number - 59
...
for i in range(range_num):
    ...
    json_label[i][0]= predict_label[0][action_index]#np.max(predict_label) # Change here to change actions predict_label[0][action_index]
#End Here
...
#If you want to generate the JSON:
#output_json_file=['Action:',output_json_file.tolist()]
#json_path = 'D:/Action.json'
